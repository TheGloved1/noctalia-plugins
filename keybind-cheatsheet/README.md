...
| Field | Value |
| --- | --- |
| ID | `gloves/keybind-cheatsheet` |
| Entries | Data service: `data`; bar widget: `keybinds`; panel: `cheatsheet` |

`gloves` is the plugin's fixed publisher namespace, not your local Linux
...
```sh
noctalia msg panel-toggle gloves/keybind-cheatsheet:cheatsheet
```
...
```ini
bind=SUPER,F1,spawn,noctalia msg panel-toggle gloves/keybind-cheatsheet:cheatsheet
```

Hyprland classic configuration:

```ini
bind = SUPER, F1, exec, noctalia msg panel-toggle gloves/keybind-cheatsheet:cheatsheet
```

Niri:

```kdl
Mod+F1 { spawn "noctalia" "msg" "panel-toggle" "gloves/keybind-cheatsheet:cheatsheet"; }
```
...
```sh
noctalia msg plugin gloves/keybind-cheatsheet:data all refresh
```
...
```sh
noctalia msg plugin gloves/keybind-cheatsheet:keybinds focused toggle
noctalia msg plugin gloves/keybind-cheatsheet:keybinds focused refresh
```

For parser development, run the fixture suite inside Noctalia:

```sh
noctalia msg plugin gloves/keybind-cheatsheet:data all self-test
```
